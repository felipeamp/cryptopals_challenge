...
constexpr char kPathToFile6[] =
  "/home/fpereira/Github/cryptopals_challenge/6.txt";

constexpr char kFileOpenError[] = "Couldn't open file \"6.txt\".";

constexpr char kCorrectKey[] = "Terminator X: Bring the noise";

constexpr char kCorrectlyDecodedMessage[] =
...
TEST(BreakRepeatingKeyXor, BreaksCorrectlyMessage) {
  std::ifstream fin;
  fin.open(kPathToFile6);
  if (!fin.is_open()) {
...
TEST(BreakRepeatingKeyXor, BreaksCorrectlyKey) {
  std::ifstream fin;
  fin.open(kPathToFile6);
  if (!fin.is_open()) {
    std::cout << kFileOpenError << std::endl;
  ...
  breakrepeatingkeyxor::BreakRepeatingKeyXor(encoded_bytes,
                                             &decoded_message,
                                             &key);
  ASSERT_STREQ(kCorrectKey, key.c_str());
}
